...
const useBiconomyContracts = () => {
  ...
  const checkAllowance = async (erc20token: string, inputAmount: any, decimals: any) => {
    ...
    try {
      ...
      console.log('Allowance--', TokenContractInstance, erc20token, allowance.toString())
      ...
      if (chainId == 42) {
        ...
        if (tokenSymbol === 'USDC') {
          ...
          const newWindow = window.open(
            'https://app.uniswap.org/#/swap?exactField=input&exactAmount=0.1&outputCurrency=0x4F96Fe3b7A6Cf9725f59d353F723c1bDb64CA6Aa',
            '_blank',
            'noopener,noreferrer'
          )
          if (newWindow) {
